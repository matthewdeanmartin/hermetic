...
def install(*, trace: bool = False) -> None:
    ...
    def _deny_eval(*a: Any, **k: Any) -> None:  # pylint: disable=unused-argument
        ...
        raise PolicyViolation(
            "dynamic code execution disabled: eval", guard="code_exec", target="eval"
        )
    ...
    def _guard_exec(*a: Any, **k: Any) -> Any:
        ...
        _trace("blocked exec")
        raise PolicyViolation(
            "dynamic code execution disabled: exec", guard="code_exec", target="exec"
        )
    ...
    def _guard_compile(
        ...
        **kwargs: Any,
        ...
        raise PolicyViolation(
            "dynamic code execution disabled: compile",
            guard="code_exec",
            target="compile",
        )
    ...
    def _guard_run_module(*a: Any, **k: Any) -> Any:
        ...
        raise PolicyViolation(
            "dynamic code execution disabled: run_module",
            guard="code_exec",
            target="run_module",
        )
    ...
    def _guard_run_path(*a: Any, **k: Any) -> Any:
        ...
        raise PolicyViolation(
            "dynamic code execution disabled: run_path",
            guard="code_exec",
            target="run_path",
        )
